...
Индексы цены используются для отображения и начальной цены очень быстро на странице списка продуктов. 
Для них добавочная информация не нужна. 
Исключение для страницы категории на энтерпрайз версии там индексы не используются.


#Компонент `Magento\Framework\Price\*` и его расширение в `Magento_Catalog` модуле
...
Условия можно добавить для адреса, для заказов пользователей, 
для веса товара, для адреса доставки.
